...
$app->get('/product/category/get/bycompany/:id', function ($id) {
    ...
    try 
    {
        ...
        $sth = $db->prepare("SELECT * 
            FROM productcategory
            WHERE CategoryCompanyID = :id order by CategoryName ASC");
...
$app->post('/product/category/add(/)', function() use($app) {
	...
	$imgs = array();
       ...
       $Category = $sth->fetchAll(PDO::FETCH_OBJ);
		 ...
		 $imgs = array();
		 if(isset($_FILES['CategoryLogo']) && !empty($_FILE["CategoryLogo"]["name"])){
